...
# class_names = [s + 1 for s in range(6)]
...
for e in epsilon: 
  train_targets_epsilon.append(util.introduce_label_noise(train_targets_actual, e, n_classes))
  val_targets_epsilon.append(util.introduce_label_noise(val_targets_actual, e, n_classes))
  test_targets_epsilon.append(util.introduce_label_noise(test_targets_actual, e, n_classes))
...
for train_targets, val_targets, test_targets, e in zip(train_targets_epsilon, val_targets_epsilon, test_targets_epsilon, epsilon):
  EMBEDDING_PATH = RESULT_PATH + '/noise_' + str(e)
  ...
  disp.plot()
  plt.tight_layout()
  plt.savefig(EMBEDDING_PATH + '/2.5_Noise_Confusion_Matrix.png')
  ...
  # model_class = torch.load('/home/sasisekhar/Desktop/SREA/modified_SREA/debug/Plane/08_04_2024__09_44_59/noise_0.3/model_class.pth')
  model_class = model_class.eval()
  model = model.eval()

  target_idx = 0
  test_metric_vals = {"AE_actual": [], "AE_predicted": [], "class_predicted": [], 'class_predicted_train': []}
  with torch.no_grad():
    ...
    for seq_true in train_dataset:
      ...
      embedding = model.encoder(seq_true).squeeze().to(device)
      y_pred = model_class(embedding)
      test_metric_vals['class_predicted_train'].append(y_pred.item())

  temp = []
  for i in test_metric_vals['class_predicted_train']:
    if(int(np.round(i*n_classes)) == 0):
      temp.append(1)
    else:
      temp.append(int(np.round(i*n_classes)))

  test_metric_vals['class_predicted_train'] = temp
  ...
  ae_metrics, class_metrics = metrics.evaluate( np.array(test_metric_vals['AE_actual']), 
                                                ...
                                                np.array(train_targets_actual),
                                                np.array(test_metric_vals['class_predicted_train']),
                                                logger,
                                                class_names,
                                                metrics=['mae', 'mse', 'rmse', 'std_ae', 'smape', 'rae', 'mbrae', 'r2', 'dtw'])
  
  temp = []
  for i in test_metric_vals['class_predicted']:
    if(int(np.round(i*n_classes)) == 0):
      temp.append(1)
    else:
      temp.append(int(np.round(i*n_classes)))

  test_metric_vals['class_predicted'] = temp
